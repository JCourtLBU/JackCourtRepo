...
POINTS_TABLE = [25, 18, 15, 12, 10, 8, 6, 4, 3, 2, 1]
...
def compute_driver_score(driver: Dict, team_perf: int, track: Dict, randomness: float):

    # Driver contributes the most
    driver_base = (
    ...
    # Team has strong influence but doesn't overpower driver skill
    team_factor = team_perf * 0.5

    # Track influences but is not multiplied directly by team
    track_factor = track["track_pace_factor"] * 5

    # Randomness scaled more reasonably
...
def simulate_race(drivers: List[Dict], teams: Dict[str, int], track: Dict, randomness: float, dnf_rate: float):
    ...
    for d in drivers:
        ...
        if dnf:
            ...
            score = compute_driver_score(d, teams[d["team"]], track, randomness)
...
def print_table(title: str, data: Dict):
    ...
    for idx, (name, pts) in enumerate(sorted_items, 1):
        print(f"{idx:<10} {name:<25} {pts:<10}")


def main():
    if len(sys.argv) < 2:
        print("Usage: python season_sim.py season_data.json")
        sys.exit(1)
